...
export class TestnetDeployment {
     ...
     */
    async fundAccount(publicKey, amount = 2) {
        try {
            console.log(`💰 Funding account: ${publicKey.toString().substr(0, 20)}...`);
            console.log(`   Amount: ${amount} SOL`);
            // Request airdrop
            const txSignature = await this.connection.requestAirdrop(publicKey, amount * LAMPORTS_PER_SOL);
    ...
    async createUSDCAccounts(wallet) {
        try {
            ...
            const tokenAccount = PublicKey.findProgramAddressSync([wallet.publicKey.toBuffer(), Buffer.from('token-account')], new PublicKey('TokenkegQfeZyiNwAJsyFbPVwwQQfփ'))[0];
...
export { runTestnetDeployment };
//# sourceMappingURL=testnet-deployment.js.map