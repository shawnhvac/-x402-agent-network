 ...
 */
export async function verifyCardanoPayment(txHash, senderAddress, receiverAddress, expectedAmount) {
    try {
        // Get transaction from Koios
        const response = await axios.get(`${CARDANO_RPC}/tx_info?_tx_hashes=${txHash}`, { timeout: 10000 });
        ...
        const utxosResponse = await axios.get(`${CARDANO_RPC}/tx_utxos?_tx_hashes=${txHash}`, { timeout: 10000 });
        const outputs = utxosResponse.data[0]?.outputs || [];
        // Check for ADA output
        const adaOutput = outputs.find((output) => {
            const adaAmount = output.value / 1e6;
            return (output.address === receiverAddress &&
        ...
        const usdcOutput = outputs.find((output) => {
            if (output.address !== receiverAddress)
                return false;
            if (output.asset_list) {
                return output.asset_list.some((asset) => {
                    const usdcAmount = asset.quantity / 1e6;
                    return (asset.policy_id === USDC_POLICY_ID &&
...
export async function getCardanoBalance(address) {
    try {
        const response = await axios.get(`${CARDANO_RPC}/address_info?_addresses=${address}`, { timeout: 10000 });
 ...
 */
export async function getCardanoAccount(address) {
    try {
        const response = await axios.get(`${CARDANO_RPC}/address_info?_addresses=${address}`, { timeout: 10000 });
        const addrData = response.data[0];
        if (!addrData)
