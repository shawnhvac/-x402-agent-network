...
export class VehicleAgent {
    constructor(agentId, walletAddress, vin, negotiationEngine) {
     ...
     */
    async autonomouslyNegotiateService(request) {
        try {
            ...
            // Request quotes
            const quotes = await this.negotiationEngine.requestQuotes(request.serviceType, request.maxBudget, request.maxTime, request.urgency);
            if (quotes.length === 0) {
            ...
            const result = await autonomousNegotiate(this.negotiationEngine, request.serviceType, request.maxBudget, request.maxTime, 70 // Min acceptable score
            );
            if (!result) {
     ...
     */
    simulateServiceExecution(negotiation) {
        console.log(`${'='.repeat(70)}`);
...
//# sourceMappingURL=vehicle-agent.js.map