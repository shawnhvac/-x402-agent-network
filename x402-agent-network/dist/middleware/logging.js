...
export function loggingMiddleware(req, res, next) {
    ...
    res.json = function (data) {
        ...
        console.log(`[${reqLog.timestamp}] ${reqLog.method} ${reqLog.path} | ${reqLog.statusCode} | ${duration}ms | ${wallet}`);
...
export function getMetrics() {
    ...
    const avgDuration = requestLogs.reduce((sum, log) => sum + (log.duration || 0), 0) /
        requestLogs.length;
    return {
        ...
        errorCount,
        successCount,
        p95ResponseTime: calculatePercentile(requestLogs.map((log) => log.duration || 0), 0.95),
        p99ResponseTime: calculatePercentile(requestLogs.map((log) => log.duration || 0), 0.99),
    };
}
/**
...
export function logPayment(paymentData) {
    console.log(`[PAYMENT] ${paymentData.timestamp} | ${paymentData.agentId} | ${paymentData.payer} | ${paymentData.amount} USDC | TX: ${paymentData.txHash.substring(0, 10)}...`);
}
/**
 * Agent execution logging
 */
export function logAgentExecution(executionData) {
    console.log(`[AGENT] ${executionData.agentId} | Exec: ${executionData.executionId} | Cost: ${executionData.cost} USDC | Duration: ${executionData.duration}ms | Success: ${executionData.success}`);
}
