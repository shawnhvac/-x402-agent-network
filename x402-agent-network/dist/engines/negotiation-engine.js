...
export class AutonomousNegotiationEngine {
    constructor(agentAddress, agentRegistry) {
     ...
     */
    async discoverServices(serviceType, maxBudget, maxTime) {
        try {
            ...
            for (const agent of allAgents) {
                ...
                for (const service of services) {
                    ...
                    if (service.name.toLowerCase().includes(serviceType.toLowerCase()) &&
     ...
     */
    async requestQuotes(serviceType, maxBudgetUSDC, maxTimeMinutes, urgency = 'medium') {
        const startTime = Date.now();
        try {
            console.log(`📋 Requesting quotes for ${serviceType}...`);
            // 1. Discover available services
            const services = await this.discoverServices(serviceType, maxBudgetUSDC, maxTimeMinutes);
            ...
            const quotes = [];
            const quotePromises = services.map((service) => this.requestQuoteFromAgent(service, quoteRequest));
     ...
     */
    async requestQuoteFromAgent(service, quoteRequest) {
        try {
    ...
    async evaluateOffers(quotes) {
        ...
        for (const quote of quotes) {
            ...
            const priceScore = maxPrice === minPrice ? 100 : ((maxPrice - quote.priceUSDC) / (maxPrice - minPrice)) * 100;
            ...
            const overallScore = reputationScore * 0.4 + priceScore * 0.35 + timeScore * 0.25;
            evaluated.push({
     ...
     */
    selectBestDeal(evaluatedOffers, minAcceptableScore = 70) {
        ...
        if (bestOffer.overallScore < minAcceptableScore) {
            console.warn(`⚠️ Best offer score ${bestOffer.overallScore} below minimum ${minAcceptableScore}`);
            return null;
        }
        console.log(`✅ Selected offer from ${bestOffer.sellerAgent}: Score ${bestOffer.overallScore}/100`);
        return bestOffer;
    }
    // ===== 5. CREATE ESCROW AUTOMATICALLY =====
    /**
     ...
     */
    async createEscrow(selectedOffer, originalQuote, milestoneDefs) {
        const startTime = Date.now();
        try {
            ...
            for (const milestone of milestones) {
                ...
                milestonePayments.push(Math.round((totalPrice * milestone.percentOfTotal) / 100));
            }
 ...
 * ALL AUTOMATICALLY
 */
export async function autonomousNegotiate(engine, quotes, minAcceptableScore = 70) {
    try {
