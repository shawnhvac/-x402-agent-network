 ...
 */
router.post('/sms-reply', express.urlencoded({ extended: false }), async (req, res) => {
 ...
 */
router.post('/ivr-response/:bookingId', express.urlencoded({ extended: false }), (req, res) => {
    ...
    if (digit === '1') {
        if (booking)
            updateBookingStatus(bookingId, 'confirmed', {}, 'ivr');
        ...
        if (booking)
            updateBookingStatus(bookingId, 'declined', {}, 'ivr');
...
router.post('/booking-status', express.json(), (req, res) => {
