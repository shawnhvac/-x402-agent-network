...
router.get('/services', (req, res) => {
    try {
        ...
        if (location && typeof location === 'string') {
            ...
            filtered = filtered.filter(s => s.location.toLowerCase().includes(locationLower) ||
 ...
 */
router.get('/services/by-category/:category', (req, res) => {
    try {
 ...
 */
router.get('/services/by-location/:location', (req, res) => {
    try {
        const { location } = req.params;
        const { limit = '50', offset = '0' } = req.query;
        const locationLower = location.toLowerCase();
        const filtered = services.filter(s => s.location.toLowerCase().includes(locationLower) ||
            s.city.toLowerCase().includes(locationLower));
        const limitNum = Math.min(parseInt(limit) || 50, 100);
        const offsetNum = parseInt(offset) || 0;
        const paginated = filtered.slice(offsetNum, offsetNum + limitNum);
...
router.post('/search', (req, res) => {
    try {
        ...
        const results = filtered.filter(s => s.name.toLowerCase().includes(queryLower) ||
            s.description.toLowerCase().includes(queryLower) ||
            s.services.some(svc => svc.toLowerCase().includes(queryLower))).slice(0, 20);
        res.json({
            success: true,
            query,
