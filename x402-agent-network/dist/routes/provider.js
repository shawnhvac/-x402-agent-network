...
if (!providerCols.includes('token'))
    try {
        db.exec('ALTER TABLE providers ADD COLUMN token TEXT');
    }
    catch { }
if (!providerCols.includes('address'))
    try {
        db.exec('ALTER TABLE providers ADD COLUMN address TEXT');
    }
    catch { }
if (!providerCols.includes('city'))
    try {
        db.exec('ALTER TABLE providers ADD COLUMN city TEXT');
    }
    catch { }
if (!providerCols.includes('lat'))
    try {
        db.exec('ALTER TABLE providers ADD COLUMN lat REAL');
    }
    catch { }
if (!providerCols.includes('lon'))
    try {
        db.exec('ALTER TABLE providers ADD COLUMN lon REAL');
    }
    catch { }
...
const router = Router();
const hashPassword = (p) => crypto.createHash('sha256').update(p + 'agentpay_salt').digest('hex');
...
function requireAuth(req, res, next) {
    ...
    const provider = db.prepare('SELECT * FROM providers WHERE token = ?').get(token);
    if (!provider)
...
router.post('/register', (req, res) => {
    try {
        ...
        if (!businessName || !email || !password)
            return res.status(400).json({ error: 'businessName, email and password required' });
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'active', ?)
    `).run(id, businessName, email.toLowerCase().trim(), phone || null, hashPassword(password), category || null, address || null, city || null, lat || null, lon || null, description || null, token, now);
...
router.post('/login', (req, res) => {
    try {
        ...
        const provider = db.prepare('SELECT * FROM providers WHERE email = ?').get(email?.toLowerCase().trim());
...
router.get('/bookings', requireAuth, (req, res) => {
    try {
        ...
        const bookings = bdb.prepare("SELECT * FROM bookings WHERE provider_phone = ? OR provider_email = ? ORDER BY created_at DESC LIMIT 50").all(req.provider.phone, req.provider.email);
...
router.get('/list', (req, res) => {
  ...
  `;
    ...
    const providers = db.prepare(sql).all(...params).map((p) => ({
        ...p,
        services: (() => { try {
