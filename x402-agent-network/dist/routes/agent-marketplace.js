...
router.get('/marketplace/info', (_req, res) => {
    res.json({
...
router.post('/marketplace/agent/register', async (req, res) => {
    try {
        const { agent_name, agent_url, capabilities, wallet_address, contact_email, description, pricing_model, supported_chains, webhook_url, mcp_endpoint, openapi_url, tags } = req.body;
        // Validate required fields
        const missing = [];
        if (!agent_name)
            missing.push('agent_name');
        if (!agent_url)
            missing.push('agent_url');
...
router.get('/marketplace/agents', (req, res) => {
    ...
    if (chain)
        agents = agents.filter((a) => a.supported_chains?.includes(chain));
    if (tag)
...
router.get('/marketplace/agent/:agentId', (req, res) => {
    const db = loadDB();
...
router.post('/marketplace/agent/verify', async (req, res) => {
    const { agent_id, api_key } = req.body;
    if (!agent_id || !api_key)
        return res.status(400).json({ success: false, error: 'agent_id and api_key required' });
    const db = loadDB();
    const keyData = db.apiKeys[api_key];
    if (!keyData || keyData.agent_id !== agent_id)
        return res.status(401).json({ success: false, error: 'Invalid api_key for this agent' });
...
router.delete('/marketplace/agent/:agentId', (req, res) => {
    const { api_key } = req.body;
    if (!api_key)
        return res.status(400).json({ success: false, error: 'api_key required in body' });
    const db = loadDB();
    const keyData = db.apiKeys[api_key];
    if (!keyData || keyData.agent_id !== req.params.agentId)
