 ...
 */
export async function getQuota(wallet) {
    try {
        const result = await pool.query(`SELECT remaining_calls FROM quotas WHERE wallet_address = $1`, [wallet]);
        if (result.rows.length === 0) {
            // Create new quota entry
            await pool.query(`INSERT INTO quotas (wallet_address, remaining_calls) VALUES ($1, $2)`, [wallet, 10]);
            return 10;
        }
 ...
 */
export async function decrementQuota(wallet) {
    try {
        const result = await pool.query(`UPDATE quotas SET remaining_calls = remaining_calls - 1 
       WHERE wallet_address = $1 RETURNING remaining_calls`, [wallet]);
        if (result.rows.length === 0) {
            // Create entry if doesn't exist
            await pool.query(`INSERT INTO quotas (wallet_address, remaining_calls) VALUES ($1, $2)`, [wallet, 9]);
            return 9;
        }
        return result.rows[0].remaining_calls;
    }
...
export async function recordPayment(data) {
    try {
        ...
        await pool.query(`INSERT INTO payments (request_id, agent_id, payer, amount, currency, chain_id, tx_hash, platform_fee, net_amount)
