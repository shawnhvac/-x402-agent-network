...
var x402HTTPResourceServer = class {
  ...
  async processHTTPRequest(context, paywallConfig) {
    ...
    try {
      ...
      console.log("[CORE-DEBUG] paymentPayload scheme:", paymentPayload?.scheme, "network:", paymentPayload?.network, "payTo:", paymentPayload?.payload?.authorization?.to?.slice?.(0,10));
      console.log("[CORE-DEBUG] accepts[0]:", JSON.stringify(paymentRequired.accepts?.[0]));
      console.log("[CORE-DEBUG] matchingRequirements:", matchingRequirements ? "FOUND" : "NULL");
      ...
      console.log("[CORE-DEBUG] verifyResult.isValid:", verifyResult.isValid, "invalidReason:", verifyResult.invalidReason);
  ...
  extractPayment(adapter) {
    ...
    console.log("[EXTRACT-DEBUG] header found:", header ? header.slice(0,40)+"..." : "NONE");
    if (header) {
      try {
        return decodePaymentSignatureHeader(header);
      } catch (error) {
        console.warn("Failed to decode PAYMENT-SIGNATURE header:", error);
        // Try direct JSON parse if base64 decode fails
        try {
          const decoded = Buffer.from(header, "base64").toString("utf8");
          return JSON.parse(decoded);
        } catch(e2) {
          try { return JSON.parse(header); } catch(e3) {}
        }
      }
    }
    return null;
