...
const CHAINS: Record<string, {
  name: string; chainId: number; symbol: string; rpc: string; explorer: string;
  usdc?: string; usdt?: string; dai?: string; nativeToken: string;
}> = {
  ethereum: {
 ...
 */
router.get('/payment/chains', (_req: Request, res: Response) => {
...
router.get('/payment/info', (req: Request, res: Response) => {
  const { chain = "base" } = req.query as { chain: string };
 ...
 */
router.post('/payment/verify-evm', async (req: Request, res: Response) => {
  try {
