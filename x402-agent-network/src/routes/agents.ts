...
router.get("/", async (req: Request, res: Response): Promise<any> => {
  try {
    ...
    const agents = (getAgents as any)(
      chainParam as any,
      req.query.minRating ? parseFloat(Array.isArray(req.query.minRating) ? req.query.minRating[0] : (req.query.minRating as string)) : undefined
    );
...
router.put("/:agentId", async (req: Request, res: Response): Promise<any> => {
  try {
    ...
    const agent = (updateAgent as any)(req.params.agentId, {
      name,
      description,
      minPayment,
      maxPayment,
