 ...
 */
router.post('/stripe/setup-customer', async (req: Request, res: Response) => {
  try {
 ...
 */
router.post('/stripe/agent-charge', async (req: Request, res: Response) => {
  try {
    const stripe = getStripe();
    const {
      customer_id, amount, currency = 'usd',
      service_name = 'AgentPay Booking',
      booking_id, agent_id, business_name
    } = req.body;
    ...
    const intent = await stripe.paymentIntents.create({
      ...
      receipt_email: (customer as any).email || undefined
    });
 ...
 */
router.get('/stripe/customer/:id', async (req: Request, res: Response) => {
  try {
    const stripe = getStripe();
    const customer = await stripe.customers.retrieve(req.params.id) as Stripe.Customer;
    ...
    res.json({
      ...
      cards: methods.data.map(m => ({
        ...
        is_default: m.id === ((customer as any).invoice_settings?.default_payment_method)
      })),
...
router.post('/stripe/checkout', async (req: Request, res: Response) => {
  try {
    const stripe = getStripe();
    const { amount, currency = 'usd', service_name = 'AgentPay Booking', booking_id, business_name,
      success_url = 'https://www.x402-agent-pay.com/payment-success',
      cancel_url  = 'https://www.x402-agent-pay.com/payment-cancel' } = req.body;
...
router.post('/stripe/webhook', (req: Request, res: Response) => {
  ...
  try {
    ...
    } else { event = req.body as Stripe.Event; }
  ...
  switch (event.type) {
    ...
    case 'payment_intent.payment_failed':
      console.log('AgentPay payment failed:', (event.data.object as any).last_payment_error?.message);
      break;
    case 'customer.created':
      console.log('New AgentPay customer:', (event.data.object as any).email);
      break;
    case 'setup_intent.succeeded':
      console.log('Card saved for customer:', (event.data.object as any).customer);
      break;
  }
