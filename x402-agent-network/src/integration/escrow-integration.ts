...
export class SmartEscrowIntegration {
  ...
  private buyer: PublicKey;
  private seller: PublicKey;
  private mint: PublicKey;
  private escrowCounter: number = 0;
  private escrowCache: Map<number, OnChainEscrow> = new Map();
   ...
   */
  async createEscrow(
    amount: number, // USDC (will be converted to lamports)
    milestones: MilestoneData[],
    ...
    try {
      ...
      const escrow: OnChainEscrow = {
        escrowId,
        buyer: this.buyer,
        seller: this.seller,
        amount: amount * 1_000_000, // Convert USDC to lamports (6 decimals)
  ...
  async completeMilestone(
    ...
    try {
      ...
      const paymentAmount = Math.round(
        (escrow.amount * milestone.percentOfTotal) / 100
      );
      ...
      console.log(`   Payment released: ${paymentAmount / 1_000_000} USDC`);
      console.log(`   Tx: ${txSignature.substr(0, 20)}...\n`);
   ...
   */
  async getEscrowState(escrowId: number): Promise<OnChainEscrow | null> {
    try {
   ...
   */
  async getEscrowBalance(escrowId: number): Promise<number> {
    const escrow = await this.getEscrowState(escrowId);
    if (!escrow) return 0;
    ...
    return (escrow.amount - releasedAmount) / 1_000_000;
  }
   ...
   */
  async watchMilestoneCompletions(
    escrowId: number,
    callback: (milestone: MilestoneData, index: number) => void
  ): Promise<void> {
    const escrow = await this.getEscrowState(escrowId);
    if (!escrow) {
      throw new Error(`Escrow not found: ${escrowId}`);
   ...
   */
  async disputeEscrow(escrowId: number, reason: string): Promise<string> {
    try {
   ...
   */
  async refundOnTimeout(escrowId: number): Promise<number> {
    try {
      ...
      // Refund to buyer
      const refundAmount = escrow.amount / 1_000_000;
  ...
  getStats(): {
    ...
    this.escrowCache.forEach((escrow) => {
      totalVolume += escrow.amount / 1_000_000;
      if (escrow.status === 'active') activeCount++;
      if (escrow.status === 'completed') completedCount++;
    });
