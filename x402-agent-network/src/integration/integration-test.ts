...
import { SmartEscrowIntegration, EscrowConfig, MilestoneData } from './escrow-integration';
import { AgentRegistry, Agent, Service } from '../engines/negotiation-engine';

// ===== TEST REGISTRY =====

class TestAgentRegistry implements AgentRegistry {
...
async function runIntegrationTests(): Promise<void> {
  ...
  const vehicle = new VehicleAgent(
    ...
    'Test Vehicle',
    new AutonomousNegotiationEngine(buyerKeyPair.publicKey.toString(), registry)
  );
  ...
  try {
    const escrowResult = await escrow.createEscrow(
      negotiationResult.selectedPrice,
      milestones,
      120
    );
  ...
  const job = mechanic.acceptEscrow(
    ...
    'tune-up',
    negotiationResult.selectedPrice,
    negotiationResult.selectedTime
  );
