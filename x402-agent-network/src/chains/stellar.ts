...
export async function verifyStellarPayment(
  ...
  try {
    ...
    const response = await axios.get(
      `${STELLAR_RPC}/transactions/${txHash}`,
      { timeout: 10000 }
    );
    ...
    const opsResponse = await axios.get(
      `${STELLAR_RPC}/transactions/${txHash}/operations`,
      { timeout: 10000 }
    );
    ...
    const xlmPayment = operations.find((op: any) => {
      return (
        op.type === 'payment' &&
        op.source_account === requesterWallet &&
        op.destination_account === treasuryWallet &&
...
export async function getStellarBalance(
  ...
  try {
    const response = await axios.get(
      `${STELLAR_RPC}/accounts/${walletAddress}`,
      { timeout: 10000 }
    );
 ...
 */
export async function getStellarAccount(walletAddress: string) {
  try {
    const response = await axios.get(
      `${STELLAR_RPC}/accounts/${walletAddress}`,
      { timeout: 10000 }
