...
export const POLYGON_USDC        = '0x3c499c542cEF5E3811e1192ce70d8cC03d5c3359' as `0x${string}`;
export const POLYGON_USDC_BRIDGED = '0x2791Bca1f2de4661ED88A30C99A7a9449Aa84174' as `0x${string}`; // USDC.e fallback
...
export function getPolygonWalletClient(privateKey: `0x${string}`) {
...
// ── Balance check ─────────────────────────────────────────────────────────────
export async function getPolygonUSDCBalance(address: `0x${string}`): Promise<string> {
...
export async function verifyPolygonPayment(
  ...
  try {
    ...
    const usdcLog = receipt.logs.find(log =>
      (log.address.toLowerCase() === POLYGON_USDC.toLowerCase() ||
...
export function getPolygonX402Config(payToWallet: `0x${string}`) {
  return {
