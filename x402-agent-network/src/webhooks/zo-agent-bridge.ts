...
export class ZoAgentBridge {
  ...
  private zoApiEndpoint = 'https://api.zo.computer/zo/ask';
  private zoAccessToken: string;
   ...
   */
  private async handleSendToZo(req: Request, res: Response) {
    try {
      ...
      const zoMessage: ZoMessage = {
        input: message,
        model_name: 'vercel:minimax/minimax-m2.7',
        conversation_id: conversationId || this.conversationId || undefined
      };

      const response = await this.axiosClient.post<ZoResponse>(this.zoApiEndpoint, zoMessage);
   ...
   * Receive response from muskox2
   */
  private async handleReceiveFromZo(req: Request, res: Response) {
    try {
   ...
   */
  private async handleStartPartnership(req: Request, res: Response) {
    try {
...
Let's build! 🚀
   ...
   */
  private async handleAgentResponse(req: Request, res: Response) {
    try {
      ...
      if (nextMessage && this.conversationId) {
        const followUp = await this.axiosClient.post<ZoResponse>(this.zoApiEndpoint, {
          input: nextMessage,
          model_name: 'vercel:minimax/minimax-m2.7',
   ...
   */
  private generatePartnershipConfirmation(actions: any): string {
    return `
...
`;
   ...
   */
  private generateAnswersToQuestions(actions: any): string {
    let response = '';
...
`;
   ...
   */
  private storeConversationMessage(msg: {
    from: string;
    to: string;
    content: string;
    type: 'REQUEST' | 'RESPONSE' | 'ACTION' | 'STATUS';
