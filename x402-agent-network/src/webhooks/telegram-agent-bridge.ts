...
interface AgentMessage {
  sender: string;
  action: 'BOOKING' | 'NEGOTIATION' | 'PAYMENT' | 'STATUS' | 'UNKNOWN';
  service?: string;
  location?: string;
  budget?: number;
  agentId?: string;
...
export class TelegramAgentBridge {
  ...
  private escrowClient: SmartEscrowClient;
  ...
  private parseAgentMessage(text: string, sender: string): AgentMessage {
    const lowerText = text.toLowerCase();
    const msg: AgentMessage = { sender, action: 'UNKNOWN' };
  ...
  private async handleBooking(msg: AgentMessage, groupId: number, messageId: number): Promise<string> {
    try {
  ...
  private async handleNegotiation(msg: AgentMessage, groupId: number, messageId: number): Promise<string> {
    try {
  ...
  private async handlePayment(msg: AgentMessage, groupId: number, messageId: number): Promise<string> {
    try {
      ...
      const mockTx: EscrowTransaction = {
        id: releaseId,
        buyerId: msg.sender,
        sellerId: 'agent_xyz',
        amount: 150,
  ...
  private async handleStatus(msg: AgentMessage): Promise<string> {
    const txArray = Array.from(this.transactions.values())
  ...
  public getTransaction(id: string): EscrowTransaction | undefined {
