 ...
 */
export async function initializeAgentsDB(mongoUrl: string) {
...
router.get('/agents', async (req: Request, res: Response) => {
  try {
    ...
    if (maxPrice) query.pricePerHour = { $lte: parseFloat(maxPrice as string) };
 ...
 */
router.get('/agents/nearby', async (req: Request, res: Response) => {
  try {
    ...
    const query: any = {
      location: {
        $near: {
          ...
          $maxDistance: parseFloat(radiusKm as string) * 1000 // Convert km to meters
        }
      }
    };
 ...
 */
router.post('/agents/:id/rating', async (req: Request, res: Response) => {
  try {
 ...
 */
router.get('/agents/stats/overview', async (req: Request, res: Response) => {
  try {
