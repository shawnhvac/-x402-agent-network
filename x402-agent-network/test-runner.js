...
// Mock the Solana web3.js imports for testing
const mockSolana = {
  Connection: class {
    constructor(url) {
      this.url = url;
    ...
    async getAccountInfo() { return { data: Buffer.alloc(0) }; }
  },
  PublicKey: class {
    constructor(key) {
      this.key = key;
    ...
    static unique() {
      return new this(`key_${Math.random().toString(36).substr(2, 9)}`);
    }
    toString() {
      return this.key;
    }
  ...
  Keypair: {
    generate() {
      return {
        publicKey: new this.PublicKey(`wallet_${Math.random().toString(36).substr(2, 9)}`)
      };
    }
  }
};
